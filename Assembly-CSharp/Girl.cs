...
public class Girl : DisplayObject
{
    ...
    public DisplayObject girlSpeechBubble
    {
        get
        {
            if (GameManager.System.Location.currentLocation == null ||
                GameManager.System.Location.currentLocation.type == LocationType.NORMAL)
            {
    ...
    protected override void OnStart()
    {
        ...
        this._dialogBackgroundNormal =
            (this._speechBubbleNormal.GetChildByName("GirlSpeechBubbleBackground") as SpriteObject);
        ...
        this._dialogBackgroundDate =
            (this._speechBubbleDate.GetChildByName("GirlSpeechBubbleBackground") as SpriteObject);
    ...
    protected override void OnUpdate()
    {
        ...
        if (this.definition.blinkHalfPiece.spriteName != string.Empty &&
            this.definition.blinkFullPiece.spriteName != string.Empty &&
            GameManager.System.Lifetime(true) - this._blinkTimestamp >= this._blinkDelay)
        {
            if (this._blinkStage < 3)
            {
                ...
                if (this._eyesExpression.expressionType != GirlExpressionType.EXCITED && !this._eyesClosed &&
                    !this._tempExpressionActive)
                {
                    this.AddGirlPieceArtToContainer(this.definition.blinkFullPiece,
                        this.GetContainerByLayer(GirlLayer.EYES));
                }
            }
            else if (this._blinkStage == 3)
            {
                ...
                {
                    this.AddGirlPieceArtToContainer(this._eyesExpression.primaryArt,
                        this.GetContainerByLayer(GirlLayer.EYES));
                }
            }
        }

        if (this._resetFace && (GameManager.System.Lifetime(true) - this._resetFaceTimestamp >= this._resetFaceDelay ||
        ...
        if (this._currentDialogLine != null)
        {
            ...
            int num = Mathf.Clamp(
                (int) Mathf.Round((float) this._currentDialogLine.GetText().Length * this.dialogReadPercent), 0,
                this._currentDialogLine.GetText().Length - 1);
            string text = this._currentDialogLineSpeechText.Substring(0, num) + "^C00000000" +
                          this._currentDialogLineSpeechText.Substring(num);
            this.dialogLabel.SetText(text);
            if (this._currentDialogLine.expressions.Count > this._dialogLineExpressionIndex + 1)
            {
                DialogLineExpression dialogLineExpression =
                    this._currentDialogLine.expressions[this._dialogLineExpressionIndex + 1];
                if (num >= dialogLineExpression.startAtCharIndex)
                {
                    this.ChangeExpression(dialogLineExpression.expression, true, dialogLineExpression.changeEyes,
                        dialogLineExpression.changeMouth, 0f);
            ...
            if (GameManager.System.Lifetime(true) - this._dialogMouthTimestamp >= 0.064f ||
                this._dialogMouthTimestamp == 0f)
            {
                ...
                if (GameManager.System.settingsVoice == SettingsVoice.BOOPS &&
                    this._alphaNumericRegex.IsMatch(this._currentDialogLineSpeechText[num].ToString()))
                {
                    GameManager.System.Audio.Play(AudioCategory.VOICE_ALT,
                        GameManager.Stage.uiGirl.girlDialogBoopSounds, -1, false, 1f, true);
                }
            }
        }

        if (GameManager.System.Location.IsLocationSettled() &&
            (GameManager.System.Lifetime(true) - this._panTimestamp >= 0.01f || this._panTimestamp == 0f) &&
            ((!this.flip && GameManager.System.Dialog.IsMainGirlShowing()) ||
             (this.flip && GameManager.System.Dialog.IsAltGirlShowing())))
        {
    ...
    public void ShowMyGirl(GirlDefinition girlDefinition, int eyeExpression, int eyebrowExpression, int mouthExpression,
        int faceExpression, int hairStyle, int outfit, bool underwear)
    {
        this.ClearGirl();
        this.definition = girlDefinition;
        this.spriteCollection =
            (Resources.Load("SpriteCollections/Girls/" + this.definition.spriteCollectionName,
        ...
        this._eyesExpression = this.definition.pieces[eyeExpression];
        this._eyebrowsExpression = this.definition.pieces[eyebrowExpression];
        this._mouthExpression = this.definition.pieces[mouthExpression];
        this._faceExpression = this.definition.pieces[faceExpression];
        this._blinkStage = 0;
        this._blinkTimestamp = GameManager.System.Lifetime(true);
        this._blinkDelay = UnityEngine.Random.Range(0.5f, 4f);
        ...
        if (GameManager.System.Location.currentLocation.type != LocationType.DATE ||
            !GameManager.System.Location.currentLocation.bonusRoundLocation)
        {
        ...
        this.AddGirlPiece(this.definition.pieces[hairStyle]);
        ...
        if (!underwear)
        {
            this.AddGirlPiece(this.definition.pieces[outfit]);
        ...
        //    IL_5D6:
        //    if (GameManager.System.Location.currentLocation.type == LocationType.DATE &&
        //        !GameManager.System.Location.currentLocation.bonusRoundLocation && girlPiece2.hideOnDates)
        //    {
        //        goto IL_692;
        //    }

        //    if (GameManager.System.Location.currentLocation.type == LocationType.DATE &&
        //        GameManager.System.Location.currentLocation.bonusRoundLocation && !girlPiece2.underwear)
        //    {
        //        goto IL_692;
        //    }
    ...
    public void ShowGirl(GirlDefinition girlDefinition)
    {
            ...
            !GameManager.System.Location.currentLocation.bonusRoundLocation)
        {
            ...
            if (GameManager.System.Location.currentLocation.type == LocationType.NORMAL)
            {
                if (girlData.metStatus == GirlMetStatus.MET &&
                    GameManager.System.Location.currentLocation.outfitOverride > 0)
                {
        ...
        if (!GameManager.System.Player.tutorialComplete && GameManager.System.Player.tutorialStep < 0 &&
            this.definition == GameManager.Stage.uiGirl.fairyGirlDef)
        {
            this.backhair.RemoveAllChildren(true);
            this.fronthair.RemoveAllChildren(true);
            this.AddGirlPieceArtToContainer(this.definition.pieces[16].primaryArt, this.outfit);
    ...
    private void AddGirlPieceArtToContainer(GirlPieceArt girlPieceArt, DisplayObject container)
    {
        ...
        if (children.Length > 0 && girlPieceArt.spriteName ==
            (container.GetChildren(false)[0] as SpriteObject).sprite.GetCurrentSpriteDef().name)
        {
            return;
        }
        ...
        SpriteObject spriteObject =
            DisplayUtils.CreateSpriteObject(this.spriteCollection, girlPieceArt.spriteName, "SpriteObject");
    ...
    public void HideBra()
    {
        ...
        this._fadeBraTweener =
            HOTween.To(p_target, 0.5f, new TweenParms().Prop("spriteAlpha", 0).Ease(EaseType.Linear));
    }

    // Token: 0x06000102 RID: 258 RVA: 0x00010534 File Offset: 0x0000E734
    public void ChangeExpression(GirlExpressionType expression, bool permanent = true, bool changeEyes = true,
        bool changeMouth = true, float tempDelay = 0f)
    {
        ...
        if (changeEyes)
        {
            if (this._blinkStage == 0)
            {
                this.AddGirlPieceArtToContainer(girlPieceByExpressionType.primaryArt,
                    this.GetContainerByLayer(GirlLayer.EYES));
            }
        ...
        this.AddGirlPieceArtToContainer(girlPieceByExpressionType.secondaryArt,
            this.GetContainerByLayer(GirlLayer.EYEBROWS));
        if (permanent)
        {
        ...
        if (changeMouth)
        {
            if (this._currentDialogLine == null)
            {
                this.AddGirlPieceArtToContainer(girlPieceByExpressionType.tertiaryArt,
                    this.GetContainerByLayer(GirlLayer.MOUTH));
            }
    ...
    private GirlPiece GetGirlPieceByExpressionType(GirlExpressionType expression)
    {
        ...
        for (int i = 0; i < this.definition.pieces.Count; i++)
        {
            if (this.definition.pieces[i].type == GirlPieceType.EXPRESSION &&
                this.definition.pieces[i].expressionType == expression)
            {
    ...
    private void ResetExpression()
    {
        ...
        this.AddGirlPieceArtToContainer(this._eyebrowsExpression.secondaryArt,
            this.GetContainerByLayer(GirlLayer.EYEBROWS));
    ...
    public void ReadDialogLine(DialogLine dialogLine, bool passive = true, bool useAltLine = false,
        bool hideSpeechBubble = false, bool useAltBaseVolume = false)
    {
        ...
        if (dialogLine != null)
        {
            ...
            this._dialogSequence = new Sequence(new SequenceParms().OnComplete(
                new TweenDelegate.TweenCallbackWParms(this.DialogSequenceComplete), new object[]
                {
                    !passive
                }));
            if (!this._dialogHideSpeechBubble)
            {
                this._dialogSequence.Insert(0f,
                    HOTween.To(this.girlSpeechBubble, 0.25f,
                        new TweenParms().Prop("childrenAlpha", 1).Ease(EaseType.EaseOutBack)));
                this._dialogSequence.Insert(0f,
                    HOTween.To(this.girlSpeechBubble.gameObj.transform, 0.25f,
                        new TweenParms().Prop("localScale", Vector3.one).Ease(EaseType.EaseOutBack)));
            }

            this._dialogSequence.Insert(0f,
                HOTween.To(this, p_duration, new TweenParms().Prop("dialogReadPercent", 1).Ease(EaseType.Linear)));
    ...
    private void DialogTextRead(bool proceedTimer, bool autoProceed)
    {
        ...
        {
            this._dialogPromptTweener = HOTween.To(this.dialogPrompt, 1f,
                new TweenParms().Prop("spriteAlpha", 1).Ease(EaseType.EaseInOutCubic).Loops(-1, LoopType.Yoyo));
        }
    }
